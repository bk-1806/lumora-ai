        ...
        <div className="flex flex-wrap gap-2">
          {missingKeywords.map((keyword) => {
            ...
            return (
              <button
                key={keyword}
                className={`group flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all duration-200 hover:border-primary/40 hover:bg-primary/5 ${isMatch ? 'border-yellow-400 bg-yellow-100 text-yellow-900' : 'border-border bg-background text-foreground'}`}
              >
                <span>{keyword}</span>
